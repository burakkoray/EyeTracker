...
Vec3b interpolate_hsv(const Vec3b color1, const Vec3b color2, const float value)
{
	...
	uchar h = saturate_cast<uchar>(cvRound((1.0 - value)*color1.val[0] + value*color2.val[0])) % 256;
	uchar s = cvRound((1.0 - value)*color1.val[1] + value*color2.val[1]);
	uchar v = cvRound((1.0 - value)*color1.val[2] + value*color2.val[2]);
...
{
	uchar b = saturate_cast<uchar>(cvRound((1.0 - value)*color1.val[0] + value*color2.val[0]));
	uchar g = saturate_cast<uchar>(cvRound((1.0 - value)*color1.val[1] + value*color2.val[1]));
	uchar r = saturate_cast<uchar>(cvRound((1.0 - value)*color1.val[2] + value*color2.val[2]));
...
bool parse_data_points(const std::string& file, std::vector<DataPoint>& out)
{
	...
	while (istrm.good() && !istrm.eof())
	{
		...
		dp.px = static_cast<unsigned int>(cvRound(x));
		dp.py = static_cast<unsigned int>(cvRound(y));
